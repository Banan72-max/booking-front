...
export function ListingCard({ listing }: { listing: Listing }) {
  return (
    <Link
      ...
      className="block rounded-xl border border-border bg-surface-2 p-4 transition-all duration-150 hover:border-primary"
    >
      <div className="mb-2 flex items-center justify-between">
        <h3 className="font-display text-sm font-bold">{listing.title}</h3>
        {listing.category && (
          <span className="text-xs text-muted">{listing.category.icon} {listing.category.name}</span>
        )}
      </div>
